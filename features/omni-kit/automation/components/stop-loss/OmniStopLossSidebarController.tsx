...
export const OmniStopLossSidebarController: FC = () => {
  ...
  return (
    <>
      {automationFormState.action &&
        [TriggerAction.Add, TriggerAction.Update].includes(automationFormState.action) && (
          <>
            <ActionPills
              items={[
                {
            ...
            <SliderValuePicker
              ...
              rightBoundry={resolvedAfterDynamicStopLossPrice}
      ...
      {automationFormState.action === TriggerAction.Remove && <>Remove sidebar content</>}
    </>
  )
}
