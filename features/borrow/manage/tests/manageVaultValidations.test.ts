...
describe('manageVaultValidations', () => {
  ...
  it(
    `validates if generate or withdraw amounts are putting vault at risk, danger or exceeding day yield`,
    () => {
  ...
  it(
    'should show meaningful message when trying to deposit to vault still being under dust limit (debt floor)',
    () => {
  ...
  it(
    'validates if next coll ratio on dai generate action is below stop loss level',
    () => {
  ...
  it(
    'validates if next coll ratio on collateral withdraw action is below stop loss level',
    () => {
      localStorage.setItem('features', '{"Automation":true}')
