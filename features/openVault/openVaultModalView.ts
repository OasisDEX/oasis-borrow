...
function OpenVaultModalView(props: OpenVaultState & { steps: number; closeModal: () => void }) {
  switch (props.stage) {
    case 'editing':
      return (
...
export function OpenVaultModal({ ilk, close }: ModalProps) {
  const { openVaultModal$ } = useAppContext()
  const openVaultModal = useObservable(openVaultModal$(ilk))
