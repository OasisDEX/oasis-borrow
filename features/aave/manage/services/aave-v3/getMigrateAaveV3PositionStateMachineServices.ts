...
import {
  createExecuteOperationExecutorTransaction,
  executeTransaction,
} from 'blockchain/better-calls/dpm-account'
...
import {
  migrationContextToEthersApprovalTransactions,
  migrationContextToEthersTransactions,
...
export function getMigrateAaveV3PositionStateMachineServices(
  ...
  return {
    ...
    runEthersApprovalTransaction: (context) => async (sendBack, _onReceive) => {
      const networkId = context.strategyConfig.networkId
      const contracts = getNetworkContracts(networkId)
      ensureEtherscanExist(networkId, contracts)
      ...
      const machine =
        createEthersTransactionStateMachine<ExecuteTransactionParameters>().withContext({
          ...
          transactionParameters: migrationContextToEthersApprovalTransactions(context),
        })

      const actor = interpret(machine, {
        id: 'ethersApprovalTransactionMachine',
      }).start()
