...
import { RAY, SECONDS_PER_YEAR } from '../utils/constants'
import { createProxyAddress$ } from '../utils/proxy'
  ...
  return proxyAddress$.pipe(
    switchMap((proxyAddress) => {
      ...
      const pie$ = everyBlock$(
        defer(() => call(context, pie)(proxyAddress as any)),
        equals,
      )
      ...
      return combineLatest(pie$, dsr$, chi$, adapterDebtAmount$).pipe(
        map(([pie, dsr, chi]) => {
