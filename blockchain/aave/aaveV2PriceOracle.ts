...
export const getAaveV2AssetsPrices: CallDef<AaveV2AssetsPricesParameters, BigNumber[]> = {
  call: (_, { contract, chainId }) =>
    contract<AaveV2PriceOracle>(getNetworkContracts(NetworkIds.MAINNET, chainId).aaveV2PriceOracle)
      .methods.getAssetsPrices,
...
export const getAaveV2OracleAssetPriceData: CallDef<{ token: string }, BigNumber> = {
  call: (args, { contract, chainId }) => {
    return contract<AaveV2PriceOracle>(
      getNetworkContracts(NetworkIds.MAINNET, chainId).aaveV2PriceOracle,
