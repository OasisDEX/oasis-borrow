...
export interface Vault<I extends Ilk> {
  /**
   * A Natural number which is used as a sequential identifier for urn addresses
   * sourced from the cdpManager contract.
   *
   * @remarks
   * On creation (open) of a vault using the cdpManager contract, two
   * actions occur:
   *
   * 1. a new id is generated from the cdpManager.cdpi variable, which
   *    increments on the creation of a new vault,
  ...
  /**
   * The "Vault.lockedCollateral" is called the "ink" in the core contracts.
   ...
   */
  collateralOraclePrice: CollateralPrice<I>
