...
describe('Replace base path in the beginning of each path', () => {
  it('happy path', () => {
    ...
    expect(replaceBasePathIfNeeded(path, basePath)).toBe('/new/path')
  })
  ...
  it(
    'should not replace basePath like occurrence which is not in the beginning of the path',
    () => {
      const basePath = '/borrow'
      const path = '/borrow/new/path/borrow'

      expect(replaceBasePathIfNeeded(path, basePath)).toBe('/new/path/borrow')
    }
  )
  ...
  it('should default to "/" if the replaced path becomes empty', () => {
    ...
    expect(replaceBasePathIfNeeded(path, basePath)).toBe('/')
  })
})
